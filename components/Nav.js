          ...
          return <Link className={`${link.path === pathname && 'text-accent'} relative flex items-center hover:text-accent transition-all duration-300`} href={link.path} key={index}>
            ...
            <div className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
              <div className='bg-white relative flex text-primary items-center p-[6px] rounded-[3px]'>
                <div className="text-[12px] leading-none font-samibold capitalize">{link.name}</div>
                {/* triangle */}
                <div className='border-solid border-l-white border-l-8 border-y-transparent border-y-[6px] border-r-0 absolute ring-2'></div>
              </div>
            </div>
            {/* icon */}
